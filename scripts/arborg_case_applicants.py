...
SERVICE_URL = (
    "https://opinstjornsysla.arborg.is/MeetingNotifications/Services.asmx?WSDL"
)
AUTH_SERVICE_URL = (
    "https://opinstjornsysla.arborg.is/OneAuthentication/Authenticate.asmx?WSDL"
)
...
api_key = auth_client.service.CreateApiKey(
    "OneOpenWebArborgExternalUser", config("ONESYSTEM_ARBORG_KEY")
)
...
async def main(db):
    async for email, case_serial in get_data():
        ...
        case = (
            db.query(Case)
            .filter(Case.municipality_id == 8200, Case.serial == case_serial)
            .first()
        )
        applicant = Applicant(
            case=case, email=email, serial=case_serial, municipality_id=8200
        )
